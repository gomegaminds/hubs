...
export const xforms = {
    ...
    clickOrGrab: function (grabDelayMs = 160) {
        return function (frame, src, dest, state = {}) {
            let click = false;
            let grab = false;
            let drop = false;

            let held = frame.get(src.rising);
            let released = frame.get(src.falling);

            let cursorDelta = frame.get("/var/mouseCoordsDelta");

            let isMoving =
                cursorDelta[0] > 0.001 || cursorDelta[1] > 0.001 || cursorDelta[0] < -0.001 || cursorDelta[1] < -0.001;
    ...
    clickAndHold: function (grabDelayMs = 160) {
        return function (frame, src, dest, state = {}) {
            ...
            if (frame.get(src.rising)) {
                ...
                state.buttonDownTime &&
                !state.grabbed &&
                performance.now() - state.buttonDownTime > grabDelayMs
            ) {
                state.grabbed = true;
                grab = true;
            }
