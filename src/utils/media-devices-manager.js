...
export default class MediaDevicesManager extends EventEmitter {
    ...
    get isMicShared() {
        return (
            this.audioTrack !== null && this.getPermissionsStatus(MediaDevices.MICROPHONE) === PermissionStatus.GRANTED
        );
    }
    ...
    deviceIdForSpeakersDeviceLabel(label) {
        return (
            this._outputDevices.filter((d) => d.label === label).map((d) => d.value)[0] || this.defaultOutputDeviceId
        );
    }
