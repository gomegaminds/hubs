...
AFRAME.registerSystem("transform-y-selected-object", {
  init() {
    this.target = null;
    this.mode = null;
...
AFRAME.registerSystem("transform-x-selected-object", {
  init() {
    this.target = null;
    this.mode = null;
    this.transforming = false;
...
AFRAME.registerComponent("transform-y-button-selector", {
  tick() {
    ...
    if (!hasHand) {
      if (this.el.components["transform-y-button"].data.mode !== TRANSFORM_MODE.CURSOR) {
        this.el.setAttribute("transform-y-button", "mode", TRANSFORM_MODE.CURSOR);
      }
    } else {
      if (this.el.components["transform-y-button"].data.mode !== TRANSFORM_MODE.PUPPET) {
        this.el.setAttribute("transform-y-button", "mode", TRANSFORM_MODE.PUPPET);
      }
    }
  }
});
AFRAME.registerComponent("transform-x-button-selector", {
  tick() {
    this.userinput = this.userinput || this.el.sceneEl.systems.userinput;
    const hasHand = this.userinput.get(paths.actions.rightHand.pose) || this.userinput.get(paths.actions.leftHand.pose);
    if (!hasHand) {
      if (this.el.components["transform-x-button"].data.mode !== TRANSFORM_MODE.CURSOR) {
        this.el.setAttribute("transform-x-button", "mode", TRANSFORM_MODE.CURSOR);
      }
    } else {
      if (this.el.components["transform-x-button"].data.mode !== TRANSFORM_MODE.PUPPET) {
        this.el.setAttribute("transform-x-button", "mode", TRANSFORM_MODE.PUPPET);
      }
    }
  }
});
...
AFRAME.registerComponent("visible-if-transforming", {
  ...
  tick(t) {
    const shouldBeVisible =
      AFRAME.scenes[0].systems["transform-x-selected-object"].transforming &&
      AFRAME.scenes[0].systems["transform-x-selected-object"].hand.el.id.indexOf(this.data.hand) !== -1;
    const visibleNeedsUpdate = this.el.getAttribute("visible") !== shouldBeVisible;
    if (visibleNeedsUpdate) {
      this.el.setAttribute("visible", AFRAME.scenes[0].systems["transform-x-selected-object"].transforming);
    }
