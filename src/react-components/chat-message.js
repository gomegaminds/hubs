...
const messageBodyDom = (body, from, fromSessionId, onViewProfile, emojiClassName) => {
    ...
    const content = (
        <div>
            {from && <div onClick={onFromClick}>{from}:</div>}
...
function renderChatMessage(body, from, allowEmojiRender) {
    const { content, emoji, multiline } = messageBodyDom(body, from, null, null, null);
    const isEmoji = allowEmojiRender && emoji;
    const el = document.createElement("div");
    document.body.appendChild(el);

    const entryDom = <div>{content}</div>;
...
export default function ChatMessage(props) {
    ...
    return (
        <div className={props.className}>
            {props.maySpawn && <button onClick={() => spawnChatMessage(props.body)} />}
            {content}
        </div>
    );
}
